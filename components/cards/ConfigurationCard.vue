<template>
  <PalletCard>
    <template #body>
      <div class="flex flex-col h-full gap-[16px]" v-if="controller?.currentProperty && controller.currentPlatform">
        ...
        <div class="border-t-[1px] border-white border-opacity-[20%] pt-[16px] grid  gap-[24px] grid-cols-4 max-lg:grid-cols-1 max-sm:grid-cols-1 max-xl:grid-cols-2 max-2xl:grid-cols-3">
          ...
          <InputText title="Количество в сутки" placeholder="Минимум 100шт" class="col-span-1"/>
          <InputText title="Продолжительность дней" placeholder="Максимум 10 дней" class="col-span-1"/>
        </div>
        <div class="border-top-box flex justify-between  gap-[8px] max-lg:flex-col ">
          ...
          <button class="very-small-text h-fit bg-[#23F0A2] rounded-[12px] text-black px-[16px] py-[12px]">
            К оплате
          </button>
        </div>
      </div>
